load.audit.files <- function(directory.audits) {
  # Detect number of cores and set up parallel cluster
  num_cores <- detectCores() - 1
  cl <- makeCluster(num_cores)
  
  # Export necessary libraries and variables
  clusterEvalQ(cl, {
    library(data.table)
    library(dplyr)
    library(stringr)
  ...
  # Debugging step: Check cluster environment
  print(clusterCall(cl, function() ls()))
  
  # Parallel file loading
  results <- parLapply(cl, list.files(directory.audits, pattern = "audit.csv", recursive = TRUE, full.names = TRUE), function(filename) {
    # Extract UUID from filename
    sp <- strsplit(filename, "\\/")[[1]]
    uuid <- sp[length(sp) - 1]
    
    # Read CSV and add uuid column
    audit <- fread(filename, stringsAsFactors = FALSE) %>% 
  ...
  res <- res %>% 
    mutate(duration = (end - start) / 1000,
           question = sapply(str_split(node, "\\/"), function(x) x[length(x)]),
...
load.audit.files.old <- function(directory.audits){
  ...
  for (filename in audit.filenames){
    ...
    audit <- data.table::fread(filename, stringsAsFactors = FALSE) %>% mutate(uuid=uuid, .before=1)
...
load.survey.data <- function(directory.survey.data, col.uuid, col.enum){
  ...
  survey.data <- survey.data %>% 
    ...
    select(uuid, enum, any_of(c("start","end", "date_assessment"))) %>% distinct()
  return(survey.data)
}


load.survey.data.xborder <- function(directory.survey.data, col.uuid, col.enum, c=country){
  filename <- list.files(directory.survey.data, pattern=".xlsx|csv", all.files=TRUE, recursive=TRUE, full.names=TRUE)
  filename <- filename %>% str_subset(country)
  ...
  survey.data <- survey.data %>% 
    mutate(uuid=!!sym(col.uuid),
           enum=str_replace_all(tolower(!!sym(col.enum)), " ","")) %>% 
    select(uuid, enum, any_of(c("start","end", "date_assessment", "members_living_community", "respondant_knowledgable"))) %>% distinct()
  return(survey.data)
}

get.summary.general <- function(audit, filter.pattern=NULL){
  
  if (!is.null(filter.pattern)) audit <- audit %>% filter(str_detect(question, filter.pattern))
...
combine_tool <- function(survey=tool, responses=choices, label_col){
  survey <- survey %>% select(type, name, label=!!sym(label_col), any_of(c("label_ar"="label::Arabic", "label_fr"="label::Francais"))) %>%
  ...
  survey %>% right_join(responses %>% distinct %>% select(list_name, name.choice=name, label.choice=!!sym(label_col), any_of(c("label.choice_ar"="label::Arabic", "label.choice_fr"="label::Francais"))) %>% 
